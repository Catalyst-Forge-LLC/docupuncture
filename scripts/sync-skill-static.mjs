#!/usr/bin/env node
/**
 * Copy DocuPuncture skills to the site static tree and write STORE zips.
 */
import {
	cpSync,
	mkdirSync,
...
for (const skillName of skillNames) {
	...
	if (!statSync(skillSrc, { throwIfNoEntry: false })?.isDirectory()) {
		console.error(`Missing skill folder: ${skillName}`);
		process.exit(1);
	}
